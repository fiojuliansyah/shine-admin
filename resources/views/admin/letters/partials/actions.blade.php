<div class="dropdown d-inline-flex">
    ...
    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton-{{ $row->id }}">
        ...
        <li>
            <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#duplicateModal-{{ $row->id }}">Duplikasi</button>
        </li>
        <li>
...
<div class="modal fade" id="duplicateModal-{{ $row->id }}" tabindex="-1" aria-labelledby="duplicateModalLabel-{{ $row->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('letters.duplicate', $row->id) }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="duplicateModalLabel-{{ $row->id }}">Duplikasi Template</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <p>Apakah Anda yakin ingin menduplikasi template <strong>{{ $row->title }}</strong>?</p>
                </div>
