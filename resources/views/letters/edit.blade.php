                ...
                <form id="letterForm" action="{{ route('letters.update', $letter->id) }}" method="POST">
                            ...
                            <select class="select2 form-select" name="site_id" required>
                                @foreach ($sites as $site)
                                    <option value="{{ $site->id }}" 
                                        {{ $letter->site_id == $site->id ? 'selected' : '' }}>
                                        {{ $site->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-select" name="type_letter_id" required>
                                @foreach ($types as $type)
                                    <option value="{{ $type->id }}" 
                                        {{ $letter->type_letter_id == $type->id ? 'selected' : '' }}>
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="p-3">
                        <textarea class="form-control" id="description" name="description" required>
                            {{ old('description', $letter->description) }}
                        </textarea>
                    </div>
                    <div class="p-3">
    ...
    tinymce.init({
        ...
        content_style: `
            ...
            @media (min-width: 840px) {
                ...
                body { background-color: #fff; box-shadow: 0 0 4px rgba(0,0,0,.15); box-sizing: border-box; margin: 1rem auto 0; max-width: 820px; min-height: calc(100vh - 1rem); padding: 2rem 6rem 2rem 6rem; }
            }
